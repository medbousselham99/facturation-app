<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Facture {{ $facture->numero_facture }}</title>
<style>
...
<body>
<div class="header">
  <h1>{{ $params->nom_entreprise ?? 'Facture' }}</h1>
  ...
  <h2 style="margin-top:20px;color:#3b82f6;">FACTURE</h2>
</div>
...
<table class="info-table">
  <tr>
    <td style="width:50%">
      ...
      <div class="value">{{ $facture->client->nom }}</div>
      @if($facture->client->adresse)<div>{{ $facture->client->adresse }}</div>@endif
      @if($facture->client->ville)<div>{{ $facture->client->ville }} {{ $facture->client->code_postal }}</div>@endif
      @if($facture->client->email)<div>{{ $facture->client->email }}</div>@endif
    </td>
    <td style="width:50%;text-align:right">
      <div class="label">Numéro</div>
      <div class="value">{{ $facture->numero_facture }}</div>
      <div class="label" style="margin-top:8px;">Date</div>
      <div class="value">{{ $facture->date_facture }}</div>
      <div class="label" style="margin-top:8px;">Échéance</div>
      <div class="value">{{ $facture->date_echeance }}</div>
    </td>
  </tr>
</table>
...
<div class="totals">
  <div class="line">Total HT: <strong>{{ number_format($facture->montant_ht, 2, ',', ' ') }} DH</strong></div>
  <div class="line">TVA (20%): <strong>{{ number_format($facture->montant_tva, 2, ',', ' ') }} DH</strong></div>
  <div class="line grand-total">Total TTC: {{ number_format($facture->montant_ttc, 2, ',', ' ') }} DH</div>
</div>
