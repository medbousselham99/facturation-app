  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        <h2>{{ isEdit ? 'Modifier' : 'Nouvelle' }} Facture</h2>
      </div>
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="form">
        <div class="form-grid">
          <div class="form-group">
            <label>Numéro Facture</label>
            <input type="text" formControlName="numero_facture" class="form-control">
          </div>
          <div class="form-group">
            <label>Client</label>
            <select formControlName="client_id" class="form-control">
              <option [value]="null">Sélectionner un client</option>
              @for (c of clients; track c.id) { <option [value]="c.id">{{ c.nom }}</option> }
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="date" formControlName="date_echeance" class="form-control">
          </div>
          <div class="form-group">
            <label>Commande liée</label>
            <input type="number" formControlName="commande_id" class="form-control" placeholder="ID commande (optionnel)">
          </div>
          <div class="form-group">
            <label>Devis lié</label>
            <input type="number" formControlName="devis_id" class="form-control" placeholder="ID devis (optionnel)">
          </div>
          <div class="form-group">
            <label>Statut</label>
...
export class FactureFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.route.snapshot.params['id']) {
      ...
      this.service.getFacture(this.itemId).subscribe(data => {
  ...
  onSubmit(): void {
    ...
    const obs = this.isEdit
      ? this.service.updateFacture(this.itemId!, this.form.value)
      : this.service.createFacture(this.form.value);
    obs.subscribe(() => this.router.navigate(['/factures']));
  }
}
