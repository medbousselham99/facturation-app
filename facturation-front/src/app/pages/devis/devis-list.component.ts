  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        <h2>Devis</h2>
        <a routerLink="/devis/new" class="btn btn-primary">+ Nouveau Devis</a>
      </div>
      <div class="search-bar">
        <input type="text" [(ngModel)]="searchTerm" (input)="filterItems()" placeholder="Rechercher un devis..." class="search-input">
      </div>
          ...
          <tbody>
            @for (item of paginatedItems; track item.id) {
              <tr>
                ...
                <td>{{ item.date_devis | date:'dd-MM-yyyy' }}</td>
                <td><span class="badge" [class]="'badge-' + item.statut">{{ item.statut }}</span></td>
                <td>{{ item.montant_ttc | number:'1.2-2' }} DH</td>
                <td class="actions">
                  <a [routerLink]="['/devis', item.id]" class="btn btn-sm btn-view">Voir</a>
                  <a [routerLink]="['/devis', item.id, 'edit']" class="btn btn-sm btn-edit">Modifier</a>
                  <button (click)="deleteItem(item.id!)" class="btn btn-sm btn-delete">Supprimer</button>
                </td>
              </tr>
            }
            @empty {
              <tr><td colspan="6" class="empty">Aucun devis trouvé</td></tr>
            }
          </tbody>
        </table>
      </div>
...
export class DevisListComponent implements OnInit {
  ...
  loadItems(): void {
    this.service.getDevis().subscribe(data => { this.items = data; this.filteredItems = data; });
  }
  ...
  filterItems(): void {
    ...
    this.filteredItems = this.items.filter(c =>
      c.numero_devis.toLowerCase().includes(term) || (c.client?.nom?.toLowerCase().includes(term) ?? false)
    );
  }

  deleteItem(id: number): void {
    if (confirm('Supprimer ce devis ?')) {
      this.service.deleteDevis(id).subscribe(() => this.loadItems());
    }
  }
}
